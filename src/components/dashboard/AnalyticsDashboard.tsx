...
export function AnalyticsDashboard({ lang, inputs }: AnalyticsDashboardProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-700 overflow-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full overflow-hidden">
        <div className="space-y-1 min-w-0 flex-1 w-full">
          <h1 className="text-xl md:text-3xl font-extrabold tracking-tight text-foreground flex items-center gap-3 flex-wrap break-words">
            {t.overview}
            <Badge variant="outline" className="text-primary border-primary/20 bg-primary/5 hidden md:flex shrink-0">V1.2 Stable</Badge>
          </h1>
        ...
        <div className="flex gap-2 w-full sm:w-auto shrink-0">
          <Button variant="outline" size="sm" className="gap-2 flex-1 sm:flex-none border-primary/10 hover:bg-primary/5">
            ...
            <span className="text-[10px] md:text-[11px] font-bold uppercase tracking-wider">{t.exportPdf}</span>
          </Button>
          <Button variant="outline" size="sm" className="gap-2 flex-1 sm:flex-none border-primary/10 hover:bg-primary/5">
            <FileJson size={14} className="text-primary" />
            <span className="text-[10px] md:text-[11px] font-bold uppercase tracking-wider">{t.exportCsv}</span>
          </Button>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
        <Card className="shadow-sm hover:shadow-md transition-all border-primary/10 overflow-hidden flex flex-col">
          ...
          <CardContent className="flex-1 min-w-0">
            <div className="text-2xl font-bold font-code text-primary/70 break-all leading-tight">
              {formatCurrency(calculatedTax)}
            </div>
            <p className="text-[10px] font-semibold text-muted-foreground/70 mt-1 uppercase tracking-tighter truncate">
              {inputs.taxRate}% DE {formatCurrency(inputs.grossIncome)}
            </p>
          </CardContent>
        </Card>
        
        <Card className="shadow-md border-primary/20 bg-primary/[0.03] ring-1 ring-primary/5 overflow-hidden flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-[10px] font-bold text-primary uppercase tracking-[0.15em]">{t.effectiveTax}</CardTitle>
          </CardHeader>
          <CardContent className="flex-1 min-w-0">
            <div className="text-3xl font-bold font-code text-primary break-all leading-none mb-1">
        ...
        <Card className="shadow-sm sm:col-span-2 lg:col-span-1 border-primary/10 overflow-hidden flex flex-col">
          <CardHeader className="pb-2">
            <CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.15em]">{t.taxBurden}</CardTitle>
          </CardHeader>
          <CardContent className="flex-1 min-w-0">
            <div className="text-2xl font-bold font-code break-all leading-tight">
              {taxBurden.toFixed(2)}%
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
        <Card className="lg:col-span-7 shadow-xl border-primary/5 overflow-hidden flex flex-col">
          <CardHeader>
          ...
          <CardContent className="h-[300px] sm:h-[400px] w-full min-w-0 px-2 sm:px-6">
                ...
                <RechartsTooltip 
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-background/95 backdrop-blur-md border rounded-xl p-3 shadow-2xl border-primary/20 max-w-[200px] overflow-hidden">
                          <p className="text-[9px] font-black uppercase text-muted-foreground mb-1 tracking-widest truncate">{payload[0].name}</p>
                          <p className="text-lg font-code font-bold text-primary break-all">{formatCurrency(payload[0].value as number)}</p>
                        </div>
                      );
                    }
                    return null;
        ...
        <Card className="lg:col-span-5 shadow-xl border-primary/10 bg-primary/[0.01] overflow-hidden flex flex-col">
          <CardHeader>
          ...
          <CardContent className="space-y-6 flex-1 min-w-0">
            <div className="p-5 rounded-2xl bg-background border border-primary/10 shadow-sm relative overflow-hidden group">
              ...
              <p className="text-sm leading-relaxed text-foreground/80 font-medium italic break-words">
                "{narrativeAnalysis}"
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-4">
                <li className="flex gap-4 text-sm items-start group min-w-0">
                  ...
                  <span className="text-muted-foreground font-medium text-xs leading-snug break-words flex-1">
                    {lang === 'es' 
            ...
            {taxBurden > 7 && (
              <div className="p-4 rounded-xl bg-destructive/10 border border-destructive/20 flex gap-4 items-center animate-pulse overflow-hidden">
                <AlertTriangle className="text-destructive shrink-0" size={20} />
                <p className="text-[10px] font-bold text-destructive uppercase leading-tight tracking-tight break-words flex-1">
                  {lang === 'es' 
